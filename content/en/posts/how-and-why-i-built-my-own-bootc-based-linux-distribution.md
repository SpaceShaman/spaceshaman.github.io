...
Installing software and preparing configuration here resembles building a regular container. I currently use Fedora 44 as the base, and some of the main programs included in the system are Sway, Fish, Alacritty, and Neovim. You can find the full and up-to-date list of software in the [SpaceOS repository](https://github.com/SpaceShaman/spaceos). The project is still evolving, so its contents may change over time.
...
Initially, the same tag was also supposed to trigger a build of the Anaconda installation ISO and attach it to a GitHub Release. The build itself worked, but the resulting file exceeded GitHub's 2 GiB limit for a single release asset. I could split an archive into several parts or find separate hosting for files that large, but that would be overengineering for a system built primarily for myself. In the end, the CI/CD pipeline publishes only container images to GHCR. Instead, the repository contains a script for building the ISO locally. It pulls the image from the `auto` channel, embeds it in the installer, and generates a SHA-256 checksum. A system installed from this media follows `auto` from the start, so it does not have to be switched manually to the update channel after installation.

A second workflow handles exactly those changes. Once a week, it checks the digest of the `fedora-bootc:44` image and compares it with the base used by the most recently published SpaceOS image. If Fedora has changed nothing, the action finishes without building anything. If the digest is new, the workflow checks out the latest release, rebuilds the image, and publishes it under a tag containing the SpaceOS version, the date, and part of the base digest. One example is `auto-v0.1.3-20260920-abcdef123456`. The `auto` tag is moved to that image, and I retain the five most recent automatic rebuilds in the registry. Images carrying release tags are not subject to this cleanup.
