...
Instalowanie programów i przygotowywanie konfiguracji przypomina tu budowanie zwykłego kontenera. Aktualnie bazuję na Fedorze 44, a wśród głównych programów dołączonych do systemu znajdują się Sway, Fish, Alacritty i Neovim. Pełną i aktualną listę oprogramowania znajdziesz w [repozytorium SpaceOS](https://github.com/SpaceShaman/spaceos). Projekt nadal się rozwija, dlatego jego zawartość może się z czasem zmieniać.
...
Początkowo ten sam tag miał również uruchamiać budowanie instalacyjnego ISO z Anacondą i dołączać je do GitHub Release. Sam proces budowania zadziałał, ale gotowy plik przekroczył limit 2 GiB dla pojedynczego zasobu wydania na GitHubie. Mógłbym dzielić archiwum na części albo szukać osobnego miejsca na tak duże pliki, tylko że przy systemie przygotowanym głównie dla mnie byłoby to przerostem formy nad treścią. Ostatecznie CI/CD publikuje więc wyłącznie obrazy kontenerowe w GHCR. W repozytorium zostawiłem za to skrypt do lokalnego zbudowania ISO. Pobiera on obraz z kanału `auto`, osadza go w instalatorze i generuje sumę SHA-256. System zainstalowany z takiego nośnika od początku śledzi `auto`, więc po instalacji nie trzeba ręcznie przełączać go na kanał aktualizacji.
